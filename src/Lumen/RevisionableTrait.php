...
trait RevisionableTrait
{
    ...
    public function getOldAttributes()
    {
        ...
        $preData = $this->prepareAttributes($attributes);
        return $preData;
    }
    ...
    {
        $new_data=$this->attributes;
        $attributes = $this->getRevisionableItems($new_data);
        $preData = $this->prepareAttributes($attributes);
        return $preData;
    }
    ...
    public function getRevisionableItems(array $values)
    {
        if (count($this->getRevisionable()) > 0) {
            $values = $this->getArrayInterSectData($values);
            $data = array_intersect_key($values, array_flip($this->getRevisionable()));
            return $data;
        }
    ...
    /**
     * original datadan getRevisionable olan keyleri getir.
     *
     * @var array
     */
    ...
    {
        return $this->hasMany('Sofa\Revisionable\Lumen\Revision', 'row_id')
            ->latest()
     ...
     *
     * @return \Sofa\Revisionable\Lumen\Revision|\Sofa\Revisionable\Lumen\Presenter|null
     */
     ...
     *
     * @param  \Sofa\Revisionable\Lumen\Revision $revision
     * @return \Sofa\Revisionable\Lumen\Presenter|\Sofa\Revisionable\Lumen\Revision
     */
    ...
    {
        return $this->hasOne('Sofa\Revisionable\Lumen\Revision', 'row_id')
            ->where('table_name', $this->getTable())
